...
vec2 gui(in vec3 pos, vec3 mouse3d = vec3(0)) {
    ...
    vec3 c = vec3(1,0,0);
    c = mod(pos+0.5*c,c)-0.5*c*c;

  
    float d1 = sdBox(p+mouse3d,vec3(sin(p.x)+cos(p.y)));

    float d2 = -sdTunnel(p,20);
    float m = 1.;

     float d =sminSDF(d1,d2);
...
vec4 render(vec2 uv = v_texcoord * 2. - 1.) {
    ...
    vec3 diffuseLit = materialColor * (diffuse * light_color + ambient_color) + light2_color * diffuse2;
